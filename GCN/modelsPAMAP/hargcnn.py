...
class GraphConvolution(Module):
    """
    Simple GCN layer, similar to https://arxiv.org/abs/1609.02907 but batched
    """
...
class HARGCNN(nn.Module):
    def __init__(self, nfeat, nhid, nadjf=None, args=None):
        ...
        _filters = 15
        self.cnn1 = nn.Conv1d(nhid, _filters, 3, padding=1)
        self.cnn2 = nn.Conv1d(_filters, _filters, 3, padding=1)
        self.cnn3 = nn.Conv1d(_filters, _filters, 3, padding=1)
#         self.cnn_feat = nn.Conv2d(3,1,3,padding=1)
        self.cnn_label = nn.Conv1d(_filters, 12, 3, padding=1, bias=False)
    ...
    def forward(self, V, A):
        ...
        x = x.transpose(1, 2)
#         print(x.shape) #torch.Size([128, 12, 3])
